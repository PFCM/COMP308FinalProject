...
// turn into fixed angles
G308_Point quaternion::toAngles() const {
    float mat[16];
    toMatrix(mat);
    // XYZ rotations
    
    G308_Point angles;
    angles.z = atan2f(mat[6],mat[10]);
    angles.y = atan2f(mat[2],sqrt(mat[6]*mat[6]+mat[10]*mat[10]));
    angles.x = atan2f(mat[1],mat[0]);
    
    return angles;
}

G308_Point quaternion::toAnglesDeg() const {
    G308_Point angles = toAngles();
    angles.x *= 57.2957795;
    angles.y *= 57.2957795;
    angles.z *= 57.2957795;
    return angles;
